...
def test_v2_schema_tolerates_json_null_for_optional_strings():
    ...
    fallback-rejected the whole response. Pin the null-coercion."""
    from reasoning.schemas.sco_basket_match_v2 import parse_or_fallback_v2
    obj = parse_or_fallback_v2({
        "physical_count_match": "yes",
        "semantic_identity_match": "uncertain",
        ...
        "missing_visible_items": [],
        "extra_visible_items": [],
        "uncertainty_reason": "items are inside closed takeaway containers",
        "video_usable": True,
        "confidence": "medium",
        "narrative": "Two food containers are visible at the checkout area.",
    })
    # Did NOT fall back to "did not match schema"
    assert "did not match schema" not in obj.narrative.lower()
    assert obj.physical_count_match == "yes"
    assert obj.semantic_identity_match == "uncertain"
    assert obj.confidence == "medium"
    assert len(obj.matched_items) == 2
    # Nulls coerced to empty strings, not propagated as None
    assert obj.matched_items[0].group_id == ""


def test_v2_schema_tolerates_null_uncertainty_reason_and_narrative():
    from reasoning.schemas.sco_basket_match_v2 import parse_or_fallback_v2
    obj = parse_or_fallback_v2({
        "physical_count_match": "yes",
    ...
    assert obj.uncertainty_reason == ""
    assert obj.narrative == ""
